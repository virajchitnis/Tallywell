...
1. **`internal/secret`** — pure crypto primitives. `EncryptBlob`/`DecryptBlob` (AES-256-GCM). `Envelope` is a multi-wrap key envelope: a random DEK is generated once; each unlock method stores a separate `wrap` that encrypts the DEK. Currently two methods are implemented: `MethodPassphrase` (Argon2id KDF) and `MethodKeychain` (raw 32-byte wrapping key stored in the OS keychain). Adding a new method (passkey) just adds a new `wrap`.
